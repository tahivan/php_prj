...
if (!nv_function_exists('nv_menu_site')) {
    function nv_block_config_menu($module, $data_block, $lang_block)
    {
        ...
        $sql = "SELECT * FROM `" . NV_PREFIXLANG . "_" . $module .
            "_menu` ORDER BY `id` DESC";
        $list = nv_db_cache($sql, 'id', $module);
        foreach ($list as $l) {
            $sel = ($data_block['menuid'] == $l['id']) ? ' selected' : '';
            $html .= "<option value=\"" . $l['id'] . "\" " . $sel . ">" . $l['title'] .
                "</option>\n";
        }
        $html .= "\t</select></td>\n";
        $html .= "</tr>";
        ...
        $html .= "<option value=\"1\" " . $sel . ">" . $lang_block['m_type1'] .
            "</option>\n";
        $sel = ($data_block['type'] == 2) ? ' selected' : '';
        $html .= "<option value=\"2\" " . $sel . ">" . $lang_block['m_type2'] .
            "</option>\n";
        $sel = ($data_block['type'] == 3) ? ' selected' : '';
        $html .= "<option value=\"3\" " . $sel . ">" . $lang_block['m_type3'] .
            "</option>\n";
        $sel = ($data_block['type'] == 4) ? ' selected' : '';
        $html .= "<option value=\"4\" " . $sel . ">" . $lang_block['m_type4'] .
            "</option>\n";
        $sel = ($data_block['type'] == 5) ? ' selected' : '';
        $html .= "<option value=\"5\" " . $sel . ">" . $lang_block['m_type5'] .
            "</option>\n";
        $sel = ($data_block['type'] == 6) ? ' selected' : '';
        $html .= "<option value=\"6\" " . $sel . ">" . $lang_block['m_type6'] .
            "</option>\n";
        $sel = ($data_block['type'] == 7) ? ' selected' : '';
        $html .= "<option value=\"7\" " . $sel . ">" . $lang_block['m_type7'] .
            "</option>\n";
        $sel = ($data_block['type'] == 8) ? ' selected' : '';
        $html .= "<option value=\"8\" " . $sel . ">" . $lang_block['m_type8'] .
            "</option>\n";
        $html .= "\t</select></td>\n";
        $html .= "</tr>";
        $html .= "<tr>";
        ...
        $html .= "</td>";
        $html .= "<td>";
        $html .= "<input type=\"checkbox\" name=\"config_is_viewdes\" value=\"1\"" . (!
            empty($data_block['is_viewdes']) ? ' checked="checked"' : '') . "/> " . $lang_block['is_viewdes_1'];
        $html .= "</td>";
        $html .= "</tr>";
        $html .= "<tr>";
        $html .= "<td>";
    ...
    function nv_block_config_menu_submit($module, $lang_block)
    {
        ...
        $return['config']['is_viewdes'] = $nv_Request->get_int('config_is_viewdes',
            'post', 0);
        $return['config']['title_length'] = $nv_Request->get_int('config_title_length',
            'post', 24);
        return $return;
    }
    function nv_bmenu_check_currit($url, $type = 0)
    {
        ...
        if ($client_info['selfurl'] == $url)
            return true;
        $_curr_url = NV_BASE_SITEURL . str_replace($global_config['site_url'] . '/', '',
            $client_info['selfurl']);
        $_url = nv_url_rewrite($url, true);
        if ($home and ($_url == nv_url_rewrite(NV_BASE_SITEURL . "index.php?" .
            NV_LANG_VARIABLE . "=" . NV_LANG_DATA) or $_url == NV_BASE_SITEURL . "index.php" or
            $_url == NV_BASE_SITEURL)) {
            return true;
        } elseif ($type == 2) {
    ...
    function nv_menu_site($block_config)
    {
        global $db;
        $list_cats = array();
        $sql = "SELECT `id`, `parentid`, `title`, `link`, `note`, `subitem`, `who_view`, `groups_view`, `module_name`, `op`, `target`, `css`, `active_type` FROM `" .
            NV_PREFIXLANG . "_menu_rows` WHERE `status`=1 AND `mid` = " . $block_config['menuid'] .
            " ORDER BY `weight` ASC";
    ...
    function nv_style_type($style, $list_cats, $block_config)
    {
        ...
        if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] .
            "/modules/menu/" . $style . ".tpl")) {
            $block_theme = $module_info['template'];
        } else {
            $block_theme = "default";
        }
        $xtpl = new XTemplate($style . ".tpl", NV_ROOTDIR . "/themes/" . $block_theme .
            "/modules/menu");
    ...
    function nv_style_type5($list_cats, $block_config)
    {
        global $module_info;
        if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] .
            "/modules/menu/top_menu_bar.tpl")) {
            $block_theme = $module_info['template'];
        } else {
            $block_theme = "default";
        }
        $xtpl = new XTemplate("top_menu_bar.tpl", NV_ROOTDIR . "/themes/" . $block_theme .
            "/modules/menu");
        $xtpl->assign('BLOCK_THEME', $block_theme);
        $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
        $i = 0;
    ...
    function nv_style_type7($list_cats, $block_config)
    {
        global $module_info;
        if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] .
            "/modules/menu/pro_dropdown.tpl")) {
            $block_theme = $module_info['template'];
        } else {
            $block_theme = "default";
        }
        $xtpl = new XTemplate("pro_dropdown.tpl", NV_ROOTDIR . "/themes/" . $block_theme .
            "/modules/menu");
        $xtpl->assign('BLOCK_THEME', $block_theme);
        $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
        $arr = array();
    ...
    function nv_style_type8($list_cats, $block_config)
    {
        global $module_info;
        if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] .
            "/modules/menu/ver_2_level.tpl")) {
            $block_theme = $module_info['template'];
        } else {
            $block_theme = "default";
        }
        $xtpl = new XTemplate("ver_2_level.tpl", NV_ROOTDIR . "/themes/" . $block_theme .
            "/modules/menu");
        $xtpl->assign('BLOCK_THEME', $block_theme);
        $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
        $arr = array();
