...
if ( ! nv_function_exists( 'nv_news_style_catid' ) )
{

    function nv_block_config_news_style_catid ( $module, $data_block, $lang_block )
    {
        ...
        $sql = "SELECT * FROM `" . NV_PREFIXLANG . "_" . $site_mods[$module]['module_data'] . "_cat` WHERE lev=1 ORDER BY `weight` ASC";
		...
		$html .= "<option value=\"4\" " . $sel . ">5 tin ngang hình lớn</option>\n";
         $sel = ($data_block['type'] == 5) ? ' selected' : '';
		$html .= "<option value=\"5\" " . $sel . ">Tin dạng list có hình</option>\n";
        $sel = ($data_block['type'] == 6) ? ' selected' : '';
		$html .= "<option value=\"6\" " . $sel . ">List tin không hình</option>\n";
        $sel = ($data_block['type'] == 7) ? ' selected' : '';
		$html .= "<option value=\"7\" " . $sel . ">Tin 2 cột</option>\n";
        $html .= "	</select></td>\n";
        $html .= "</tr>";
        return $html;
    }

    function nv_block_config_news_style_catid_submit ( $module, $lang_block )
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_news_style_catid ( $block_config )
    {
		global $module_array_cat, $op, $db, $array_op ,$module_info, $my_head, $lang_module, $site_mods,$global_config;
		$parentid = $module_array_cat[$block_config['catid']]['parentid'];
		$catid = $module_array_cat[$parentid]['catid'];
		$alias = $module_array_cat[$catid]['alias'];
		$alias_2 = $module_array_cat[$block_config['catid']]['alias'];
		...
		if($array_op)
		{
			...
			if($array_op[0] == $alias or ($op=="detail" and $array_op[0] == $alias) or  $alias_2 == $array_op[0])
			{
				$html = "";
				...
				elseif($block_config['type'] == 4){	
					$module = $block_config['module'];
					$sql = "SELECT id, listcatid, publtime, exptime, title, alias, hometext, homeimgthumb, homeimgfile FROM `" . NV_PREFIXLANG . "_" . $site_mods[$module]['module_data'] . "_" . $block_config['catid'] . "`  WHERE inhome='1' AND  publtime < " . NV_CURRENTTIME . " AND (exptime=0 OR exptime >" . NV_CURRENTTIME . ") ORDER BY publtime DESC LIMIT 0,5";
				...
				elseif($block_config['type'] == 7){	
					...
					if ( ! empty( $list ) )
					{
						if ( file_exists( NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/news/block_style7.tpl" ) )
						{
							$block_theme = $module_info['template'];
						}
						else
						...
						$xtpl = new XTemplate( "block_style7.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/news" );
						$xtpl->assign( 'linksite', NV_BASE_SITEURL."themes/".$global_config['site_theme'] );
						foreach ( $list as $l )
						{
							$l['title1'] = nv_clean60( $l['title'] , 30 );
							$arr_catid = explode( ',', $l['listcatid'] );
							if(isset($arr_catid[1]))
							{
								$link = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module . "&amp;" . NV_OP_VARIABLE . "=" . $module_array_cat[$arr_catid[1]]['alias'] . "/" . $l['alias'] . "-" . $l['id'];
...
if ( defined( 'NV_SYSTEM' ) )
{
    ...
    if ( isset( $site_mods[$module] ) )
    {
        ...
        $content = nv_news_style_catid( $block_config );
    }
}
