...
if( $allowed )
{
	...
	$sql = "SELECT `id`, `" . NV_LANG_DATA . "_title`, `" . NV_LANG_DATA . "_alias`, `homeimgthumb`, `addtime`, `product_code`, `product_price`, `product_discounts`, `money_unit`, `showprice`, `" . NV_LANG_DATA . "_hometext`, `" . NV_LANG_DATA . "_address`, `" . NV_LANG_DATA . "_warranty`, `" . NV_LANG_DATA . "_promotional` FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE id!=" . $id . " AND `listcatid` = " . $data_content['listcatid'] . " AND `status`=1 ORDER BY ID DESC LIMIT " . ( $pro_config['per_row'] * 2 );
	$result = $db->sql_query( $sql );

	$data_others = array();
	while ( list( $_id, $title, $alias, $homeimgthumb, $addtime, $product_code, $product_price, $product_discounts, $money_unit, $showprice, $hometext, $address, $warranty, $promotional ) = $db->sql_fetchrow( $result ) )
	{
	...
	if ( ! empty( $_SESSION[$module_data . '_proview'] ) )
	{
		...
		$sql = "SELECT `id`, `" . NV_LANG_DATA . "_title`, `" . NV_LANG_DATA . "_alias`, `homeimgthumb`, `addtime`, `product_code`, `product_price`, `product_discounts`, `money_unit`, `showprice`, `" . NV_LANG_DATA . "_hometext`, `" . NV_LANG_DATA . "_address`, `" . NV_LANG_DATA . "_warranty`, `" . NV_LANG_DATA . "_promotional` FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE `id` IN ( " . $arrtempid . ") AND `status`=1 ORDER BY `id` DESC LIMIT ".( $pro_config['per_row'] * 2 );
		$result = $db->sql_query( $sql );
