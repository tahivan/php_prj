...
if( empty( $contents ) )
{
	...
	if( $pro_config['home_view'] == "view_home_all" )
	{
		...
		while ( list( $id, $listcatid, $publtime, $title, $alias, $hometext, $address, $warranty, $promotional, $homeimgalt, $homeimgthumb, $product_code, $product_price, $product_discounts, $money_unit, $showprice ) = $db->sql_fetchrow( $result ) )
		{
	...
	elseif( $pro_config['home_view'] == "view_home_cat" )
	{
		foreach ( $global_array_cat as $catid_i => $array_info_i )
		{
			if ( $array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0 )
			{
				...
				$sql = "SELECT SQL_CALC_FOUND_ROWS `id`, `publtime`, `" . NV_LANG_DATA . "_title`, `" . NV_LANG_DATA . "_alias`, `" . NV_LANG_DATA . "_hometext`, `" . NV_LANG_DATA . "_address`, `" . NV_LANG_DATA . "_warranty`, `" . NV_LANG_DATA . "_promotional`, `homeimgalt`, `homeimgthumb`, `product_code`, `product_price`, `product_discounts`, `money_unit`, `showprice` FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE `listcatid` IN (" . implode( ",", $array_cat ) . ") AND `inhome`=1 AND `status`=1 ORDER BY `id` DESC LIMIT 0," . $array_info_i['numlinks'];
				...
				while ( list( $id, $publtime, $title, $alias, $hometext, $address, $warranty, $promotional, $homeimgalt, $homeimgthumb, $product_code, $product_price, $product_discounts, $money_unit, $showprice ) = $db->sql_fetchrow( $result ) )
				{
					$thumb = explode( "|", $homeimgthumb );
					if ( ! empty( $thumb[0] ) and ! nv_is_url( $thumb[0] ) )
					{
